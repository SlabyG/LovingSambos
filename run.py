...
def get_sales_data():
    ...
    by commas. The loop will repeateadly request data, untill it is valid.
    """
    while True:
...
def validate_data(values):
    ...
    try:
        ...
        if len(values) != 6:
            raise ValueError(
                f"Exactly 6 values reqiuered, you provided {len(values)}"
            )
...
def update_worksheet(data, worksheet):
    """
    Rceives a list of integers to be inserted into a worksheet 
...
def calculate_surplus_data(sales_row):
    """
    Compare Sales with stock calculator the surplus for each item type.
...
def get_stock_values(data):
    """
    This function will get the stock values 
    """
